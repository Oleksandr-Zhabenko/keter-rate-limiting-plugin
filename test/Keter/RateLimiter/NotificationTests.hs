...
withCapturedOutput :: String -> IO () -> IO Text
...
tests = testGroup "Keter.RateLimiter.Notifications Tests"
  [ testCase "Console notifier logs correctly" $ do
      ...
      output <- withCapturedOutput "Console notifier logs correctly" $ notify consoleNotifier throttleName user 5
      ...
      let message = rest
      let expectedMessage = throttleName <> " blocked " <> user <> " (limit: 5)\n"
      ...
      when (not isValidTimestamp && not (T.null trimmedTimestamp)) $
        putStrLn $ "Invalid timestamp in Console notifier: length=" ++ show (T.length trimmedTimestamp) ++ ", timestamp=" ++ show (T.unpack trimmedTimestamp) ++ ", raw timestamp=" ++ show (T.unpack timestamp) ++ ", rest=" ++ show (T.unpack rest) ++ ", message=" ++ show (T.unpack message) ++ ", raw output=" ++ show (T.unpack output)
      assertBool ("Console notifier rest does not start with throttleName: rest=" ++ T.unpack rest ++ ", expected throttleName=" ++ T.unpack throttleName) (T.isPrefixOf throttleName rest)
      assertEqual "Console output message mismatch" expectedMessage message

  , testCase "Console WAI notifier logs correctly" $ do
      let req = defaultRequest { requestMethod = BS8.pack "GET", rawPathInfo = BS8.pack "/api/users", rawQueryString = BS8.pack "?limit=10", remoteHost = SockAddrInet 8080 (tupleToHostAddress (192,168,1,100)) }
      let throttleName = "apiRequests"
      output <- withCapturedOutput "Console WAI notifier logs correctly" $ notifyWAI consoleWAINotifier throttleName req 50
      let (timestamp, rest) = if T.isInfixOf " - " output then T.breakOnEnd " - " output else (T.empty, output)
      let trimmedTimestamp = T.strip $ T.dropEnd 2 timestamp
      let message = rest
      let expectedMessage = throttleName <> " blocked GET /api/users?limit=10 from 192.168.1.100:8080 (limit: 50)\n"
      let isValidTimestamp = T.length trimmedTimestamp == 19 && T.all (`T.elem` ("0123456789-: " :: Text)) trimmedTimestamp
      when (not isValidTimestamp && not (T.null trimmedTimestamp)) $
        putStrLn $ "Invalid timestamp in Console WAI notifier: length=" ++ show (T.length trimmedTimestamp) ++ ", timestamp=" ++ show (T.unpack trimmedTimestamp) ++ ", raw timestamp=" ++ show (T.unpack timestamp) ++ ", raw output=" ++ show (T.unpack output)
      ...
      assertEqual "Console WAI output message mismatch" expectedMessage message
  ...
  , testCase "No-op notifier produces no output" $ do
      let user = "user123"
      output <- withCapturedOutput "No-op notifier produces no output" $ notify noopNotifier "loginAttempts" user 5
      assertEqual "No-op notifier should produce no output" T.empty output

  , testCase "No-op WAI notifier produces no output" $ do
      let req = defaultRequest { requestMethod = BS8.pack "GET", rawPathInfo = BS8.pack "/api/users", rawQueryString = BS8.pack "?limit=10", remoteHost = SockAddrInet 8080 (tupleToHostAddress (192,168,1,100)) }
      output <- withCapturedOutput "No-op WAI notifier produces no output" $ notifyWAI noopWAINotifier "apiRequests" req 50
      assertEqual "No-op WAI notifier should produce no output" T.empty output
